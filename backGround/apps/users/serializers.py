...
class UserProfileSerializer(serializers.ModelSerializer):
    class Meta:
        model = User
        fields = ("name", "gender", "birthday", 'id')
...
class UserDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ("name", "gender", "birthday", "typ")
...
class UserRegSerializer(serializers.ModelSerializer):
    ...
    gender = serializers.ChoiceField(choices=(("male", u"男"), ("female", "女")), default="male",
                                     allow_blank=False, label="性别", help_text="性别")
    name = serializers.CharField(label="用户名", help_text="用户名", required=True, allow_blank=False,
                                 max_length=30)
    birthday = serializers.DateField(label="生日", help_text="生日", required=True, format='YYYY-MM-DD')
    password = serializers.CharField(
        style={'input_type': 'password'}, help_text="密码", label="密码", write_only=True, required=True, allow_blank=False
    )
    email = serializers.CharField(label="邮箱", help_text="邮箱", required=True, allow_blank=False,
                                  max_length=30)
    typ = serializers.CharField(label="病症", help_text="病症", required=True, allow_blank=False,
                                 max_length=500)
    ...
    class Meta:
        model = User
        fields = ('name', 'password', 'email', 'code', 'gender', 'typ', 'birthday',)
